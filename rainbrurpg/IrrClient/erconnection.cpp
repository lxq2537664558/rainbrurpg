...
/** The event Receiver main loop
  ...
  */
bool RainbruRPG::Events::erConnection::OnEvent(SEvent event){
  ...
  if (event.EventType == EET_GUI_EVENT)	    {
    ...
    case EGET_BUTTON_CLICKED:
      if (id == 101){ //Connect bouton
	LOGI("CONNECT button has been clicked");
	...
	if (root){
	  if (controlBefore(root)){
	    ...
	    //	    bool success=GameEngine::getSingleton().connectUser(userText, 
	    //						pwdText);
	...
	return true;
      ...
      if (id == 102){ //Create account bouton
	LOGI("CREATE ACCOUNT button was clicked");
	GameEngine::getSingleton().changeState(ST_CREATE_ACCOUNT);
	return true;
      }
      if (id == 103){ //Change password bouton
	LOGI("CHANGE PWD button was clicked");
	GameEngine::getSingleton().changeState(ST_CHANGE_PWD);

	return true;
      }
      if (id == 104){ //Lost password bouton
	LOGI("LOST PWD button was clicked");
	return true;
      }
...
/** Makes some control before asking connection
  ...
  */
bool RainbruRPG::Events::erConnection::controlBefore(IGUIElement* root){
  bool ret=true;
  IGUIEnvironment* env=GameEngine::getSingleton().getIrrlichtGui();
