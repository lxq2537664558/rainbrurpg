...
/** The event Receiver main loop
  ...
  */
bool RainbruRPG::Events::erCreateAccount::OnEvent(SEvent event){
  ...
  if (event.EventType == EET_GUI_EVENT)	    {
	...
	LOGI("CREATE ACCOUNT button has been clicked");
...
/** Makes some control before asking connection
  ...
  */
bool RainbruRPG::Events::erCreateAccount::controlBefore(IGUIElement* root){
  bool ret=true;
  IGUIEnvironment* env=GameEngine::getSingleton().getIrrlichtGui();
